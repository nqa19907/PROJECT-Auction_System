...
public class SerializedDatabase {
  ...
  public SerializedDatabase(final SerializedDatabaseConfig config) {
    ...
    this.userRepository = new UserRepository(config.usersFile());
    this.itemRepository = new ItemRepository(config.itemsFile());
    this.auctionRepository = new AuctionRepository(config.auctionsFile());
    this.bidTransactionRepository =
        new BidTransactionRepository(config.bidTransactionsFile());
   ...
   *
   * <p>Nên dùng hàm này cho các thao tác cần cập nhật nhiều repository cùng lúc,
   * ví dụ: đặt giá thành công thì vừa cập nhật Auction, vừa lưu BidTransaction.
   *
